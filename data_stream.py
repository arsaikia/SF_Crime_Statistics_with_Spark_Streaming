...
# Spark udf to convert time to YYYYmmDDhh format
@psf.udf(StringType())
def udf_convert_time(timestamp):
    my_date = parse_date(timestamp)
    return str(my_date.strftime('%y%m%d%H'))
...
def run_spark_job(spark):
    ...
    # Extracts the correct column from the kafka input resources
    ...
    # udf to convert timestamp to right format on a call_date_time column
    converted_df = counts_df.withColumn(
        "call_date_time", udf_convert_time(counts_df.call_date_time))

    # Aggregations using windows function to see how many calls occurred in 2 day span
